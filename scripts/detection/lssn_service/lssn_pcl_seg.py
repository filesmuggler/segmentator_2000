...
class LssnPclSegServer:
    ...
    def handle_data(self, req):
        objects = []
        print(len(req.mask_list))
        for mask in req.mask_list:
            masked_depth = ros_numpy.numpify(req.depth_image) * np.mean(ros_numpy.numpify(mask), axis=2)
            masked_depth_f = masked_depth[(masked_depth > 0)]

            z = masked_depth_f.mean()
            cols, rows = np.nonzero(masked_depth)
            cx, cy = req.cam_info.K[2], req.cam_info.K[5]
            fx, fy = req.cam_info.K[0], req.cam_info.K[4]
            u, v = int(rows.mean()), int(cols.mean())
            # z = masked_depth[u,v]
            u_min, u_max = rows.min(), rows.max()
            v_min, v_max = cols.min(), cols.max()
            new_u = (u_max + u_min) / 2
            new_v = (v_max + v_min) / 2
            ...
            new_y = z * (new_v - cy) / fy
            new_x = -z * (new_u - cx) / fx
            # print("new x:",new_x," new y:",new_y," new z:",z)
